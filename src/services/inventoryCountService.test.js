...
describe('reconcile_inventory_count_finish SQL contract', () => {
  ...
  it('implements Strategy 4 using sessions.snapshot_at with no item-created_at fallback', () => {
    ...
    expect(functionBody).toContain("m.type in ('receive', 'transfer_in')")
    expect(functionBody).toContain("m.type in ('usage', 'transfer_out')")
    expect(functionBody).toContain("m.type = 'adjustment'")
    expect(functionBody).toContain("m.type in ('receive', 'usage', 'adjustment', 'transfer_out', 'transfer_in')")
    ...
    expect(functionBody).toContain('coalesce(bal.quantity, 0)')
    expect(functionBody).toContain('stock_item_location_balances bal')
    expect(functionBody).toContain('bal.location_key = i.storage_location')
...
describe('build_inventory_count_snapshot SQL contract (P8.3.9c)', () => {
  ...
  it('freezes location balance quantity by session location_key (P8.29.8)', () => {
    expect(functionBody).toContain('from public.stock_item_location_balances b')
    expect(functionBody).toContain('inner join public.stock_items si')
    expect(functionBody).toContain('b.location_key')
    expect(functionBody).toContain('coalesce(b.quantity, 0)')
    expect(functionBody).toContain('b.location_key in (')
    expect(functionBody).not.toContain('coalesce(si.current_quantity, 0)')
    expect(functionBody).not.toContain('si.storage_location in (')
  })
})
...
describe('post_inventory_count_finish SQL contract (P8.5.3 atomic post)', () => {
  ...
  it('locks session and location balances, then reconciles via shared Strategy 4 only', () => {
    expect(functionBody).toContain('for update of s')
    expect(functionBody).toContain('for update of si')
    expect(functionBody).toContain('for update of b')
    expect(functionBody).toContain('stock_item_location_balances b')
    expect(functionBody).toContain('order by i.item_id, i.storage_location')
    expect(functionBody).toContain('public.reconcile_inventory_count_finish(p_workspace_id, p_session_id)')
    expect(functionBody).toContain("v_session.status is distinct from 'counting_complete'")
  ...
  it('applies location-aware adjustments, balance updates, aggregate refresh, and finalization', () => {
    ...
    expect(functionBody).toContain('quantity = v_resulting_quantity_after_post')
    ...
    expect(functionBody).toContain("'inventory_count_post'")
    expect(functionBody).toContain('origin_workflow')
    ...
    expect(functionBody).toContain("v_line ->> 'storage_location'")
    ...
    expect(functionBody).toContain('inventory_count_post_negative_balance_rejected')
