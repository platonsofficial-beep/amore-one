...
const LEAVE_REQUEST_ID = 'leave-pending-1'

const EXPECTED_APPROVE_LEAVE_RPC_RESULT = {
  id: LEAVE_REQUEST_ID,
  workspaceId: WORKSPACE_ID,
  employeeId: EMPLOYEE_ID,
  status: 'approved',
...
const APPROVE_LEAVE_RPC_ERROR_CASES = [
...
describe('leaveService', () => {
  ...
  describe('approveLeaveRequest', () => {
    ...
    it('calls approve_leave_request with the exact RPC name', async () => {
      supabaseMocks.setRpcResult({
        data: [buildLeaveRpcRecord({ id: LEAVE_REQUEST_ID, status: 'approved' })],
        error: null,
      })

      await approveLeaveRequest(WORKSPACE_ID, LEAVE_REQUEST_ID)

      expect(supabaseMocks.rpc).toHaveBeenCalledWith('approve_leave_request', expect.any(Object))
    })
    ...
    it('sends exact two-parameter RPC payload', async () => {
      ...
      expect(supabaseMocks.rpc).toHaveBeenCalledWith('approve_leave_request', {
        p_workspace_id: WORKSPACE_ID,
    ...
    it('trims workspace id', async () => {
      ...
      expect(supabaseMocks.rpc).toHaveBeenCalledWith('approve_leave_request', expect.objectContaining({
        p_workspace_id: WORKSPACE_ID,
      }))
    })
    ...
    it('maps a one-row array response to camelCase', async () => {
      ...
      const result = await approveLeaveRequest(WORKSPACE_ID, LEAVE_REQUEST_ID)

      expect(result).toEqual(EXPECTED_APPROVE_LEAVE_RPC_RESULT)
    ...
    it('rejects missing workspace before RPC', async () => {
      await expect(approveLeaveRequest('', LEAVE_REQUEST_ID)).rejects.toThrow(
        'Workspace is required to approve leave.',
      )
    ...
    it('rejects missing leave request id before RPC', async () => {
      await expect(approveLeaveRequest(WORKSPACE_ID, '')).rejects.toThrow(
        'Leave request is required to approve leave.',
      )
    ...
    it('throws when RPC returns an empty array', async () => {
      ...
      await expect(approveLeaveRequest(WORKSPACE_ID, LEAVE_REQUEST_ID)).rejects.toThrow(
    ...
    it.each(APPROVE_LEAVE_RPC_ERROR_CASES)(
      ...
      async (rpcCode, friendlyMessage) => {
        ...
        await expect(approveLeaveRequest(WORKSPACE_ID, LEAVE_REQUEST_ID)).rejects.toThrow(friendlyMessage)
      },
    )
    ...
    it('logs and throws unknown Supabase RPC errors with a message', async () => {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        '[leaveService] approveLeaveRequest error:',
        expect.objectContaining({ message: 'unexpected database failure' }),
      )
    ...
    it('logs and throws a fallback when unknown Supabase RPC errors have no message', async () => {
      ...
      await expect(approveLeaveRequest(WORKSPACE_ID, LEAVE_REQUEST_ID)).rejects.toThrow(
        'Unable to approve the leave request right now.',
      )
